...
#
# class ScriptManager:
#     """Класс для работы со списком скриптов (загрузка, сохранение, поиск)."""
#
...
#             found = False
#             if search_name and query in s["name"].lower():
#                 found = True
#             if search_desc and query in s["description"].lower():
#                 found = True
#             if search_code:
#                 try:
...
#     """Диалог для добавления нового скрипта."""
#
#     def __init__(self, parent, manager: ScriptManager, refresh_callback):
#         super().__init__(parent)
#         self.title("Добавить скрипт")
...
#         tk.Label(self, text="Название:").pack(anchor="w", padx=5, pady=2)
#         tk.Entry(self, textvariable=self.name_var).pack(fill=tk.X, padx=5)
#
#         tk.Label(self, text="Описание:").pack(anchor="w", padx=5, pady=2)
#         self.desc_text = tk.Text(self, height=4)
#         self.desc_text.pack(fill=tk.X, padx=5)
#
#         tk.Label(self, text="Файл:").pack(anchor="w", padx=5, pady=2)
#         tk.Entry(self, textvariable=self.path_var).pack(fill=tk.X, padx=5)
#
#         btn_frame = tk.Frame(self)
#         btn_frame.pack(fill=tk.X, pady=5)
#         tk.Button(btn_frame, text="Выбрать файл", command=self.choose_file).pack(side=tk.LEFT, padx=5)
#         tk.Button(btn_frame, text="Создать новый", command=self.create_file).pack(side=tk.LEFT, padx=5)
#
#         tk.Label(self, text="Язык:").pack(anchor="w", padx=5, pady=2)
#         tk.OptionMenu(self, self.language_var, "python", "bash", "powershell").pack(fill=tk.X, padx=5)
#
#         tk.Label(self, text="Режим:").pack(anchor="w", padx=5, pady=2)
#         tk.OptionMenu(self, self.mode_var, "script", "function").pack(fill=tk.X, padx=5)
#
#         tk.Label(self, text="Параметры:").pack(anchor="w", padx=5, pady=2)
#         self.params_box = tk.Listbox(self, height=5)
#         self.params_box.pack(fill=tk.BOTH, padx=5, pady=2, expand=True)
#
#         p_btn_frame = tk.Frame(self)
#         p_btn_frame.pack(fill=tk.X, pady=5)
#         tk.Button(p_btn_frame, text="Добавить", command=self.add_param_dialog).pack(side=tk.LEFT, padx=5)
#         tk.Button(p_btn_frame, text="Удалить", command=self.delete_param).pack(side=tk.LEFT, padx=5)
#
#         tk.Button(self, text="Сохранить", command=self.save_script).pack(pady=10)
#
...
#         win.title("Добавить параметр")
#         win.geometry("300x200")
#
#         name_var = tk.StringVar()
#         type_var = tk.StringVar(value="строка")
#
#         tk.Label(win, text="Имя параметра:").pack(anchor="w", padx=5, pady=2)
#         tk.Entry(win, textvariable=name_var).pack(fill=tk.X, padx=5)
#
#         tk.Label(win, text="Тип:").pack(anchor="w", padx=5, pady=2)
#         tk.OptionMenu(win, type_var, "строка", "число", "список строк", "список чисел", "файловый путь").pack(fill=tk.X, padx=5)
#
#         def save_param():
#             param = {"name": name_var.get(), "type": type_var.get()}
#             self.params.append(param)
#             self.params_box.insert(tk.END, f"{param['name']} ({param['type']})")
#             win.destroy()
#
#         tk.Button(win, text="Сохранить", command=save_param).pack(pady=5)
#
...
#             messagebox.showerror("Ошибка", "Укажите имя и файл скрипта")
#             return
#
#         script_data = {
#             "name": name,
...
#         }
#
#         self.manager.add_script(script_data)
...
#         self.root = root
#         self.root.title("Script Manager (Таблица)")
...
#     def create_widgets(self):
#         # Настройка сетки
#         self.root.rowconfigure(0, weight=1)   # поиск
#         self.root.rowconfigure(1, weight=2)   # таблица
#         self.root.rowconfigure(2, weight=2)   # предпросмотр
#         self.root.rowconfigure(3, weight=0)   # кнопки
...
#
#         tk.Entry(search_frame, textvariable=self.search_var, font=("Arial", 14)).pack(fill=tk.X, padx=5, pady=5)
#
#         chk_frame = tk.Frame(search_frame)
#         chk_frame.pack(fill=tk.X, padx=5)
#         tk.Checkbutton(chk_frame, text="Название", variable=self.chk_name_var).pack(side=tk.LEFT)
#         tk.Checkbutton(chk_frame, text="Описание", variable=self.chk_desc_var).pack(side=tk.LEFT)
#         tk.Checkbutton(chk_frame, text="Код", variable=self.chk_code_var).pack(side=tk.LEFT)
#         tk.Button(chk_frame, text="Поиск", command=self.refresh_list).pack(side=tk.RIGHT, padx=5)
#
#         # Таблица
#         self.tree = ttk.Treeview(self.root, columns=("name", "lang", "mode"), show="headings")
...
#
#         tk.Label(bottom_frame, textvariable=self.desc_text, wraplength=950, justify="left").grid(row=0, column=0, sticky="w", padx=5)
#
#         self.preview = tk.Text(bottom_frame, wrap="word")
#         self.preview.grid(row=1, column=0, sticky="nsew", padx=5, pady=5)
#
#         # Кнопки
#         btn_frame = tk.Frame(self.root)
#         btn_frame.grid(row=3, column=0, sticky="ew", pady=5)
#         tk.Button(btn_frame, text="Добавить скрипт", command=self.add_script_dialog).pack(side=tk.LEFT, padx=5)
#         tk.Button(btn_frame, text="Открыть в редакторе", command=self.open_editor).pack(side=tk.LEFT, padx=5)
#         tk.Button(btn_frame, text="Запустить", command=self.run_script).pack(side=tk.LEFT, padx=5)
#
#     def refresh_list(self):
#         for row in self.tree.get_children():
#             self.tree.delete(row)
#
#         scripts = self.manager.search(
#             self.search_var.get(),
#             self.chk_name_var.get(),
#             self.chk_desc_var.get(),
#             self.chk_code_var.get()
#         )
#
#         for s in scripts:
#             self.tree.insert(
...
#                 tk.END,
#                 iid=s["name"],
#                 values=(s["name"], s.get("language", "python"), s.get("mode", "script"))
#             )
...
#             return
#         name = sel[0]
#         script = next(s for s in self.manager.scripts if s["name"] == name)
#         self.desc_text.set(script["description"])
#         try:
...
#             self.preview.delete(1.0, tk.END)
#             self.preview.insert(tk.END, f"[Ошибка чтения файла: {e}]")
#
...
#         script = next(s for s in self.manager.scripts if s["name"] == name)
#
#         if not script.get("params"):
#             self._execute_script(script, [])
#             return
#
#         win = tk.Toplevel(self.root)
#         win.title("Параметры запуска")
#
#         entries = {}
#         for i, param in enumerate(script["params"]):
#             tk.Label(win, text=f"{param['name']} ({param['type']})").grid(row=i, column=0, padx=5, pady=2, sticky="w")
#             entry = tk.Entry(win)
#             entry.grid(row=i, column=1, padx=5, pady=2)
#             entries[param["name"]] = (entry, param["type"])
#
#         def run_with_params():
#             args = []
#             for name, (entry, ptype) in entries.items():
#                 val = entry.get().strip()
#                 if ptype == "число":
#                     val = str(int(val))
...
#                     val = " ".join([str(int(x)) for x in val.split(",")])
#                 elif ptype == "файловый путь":
#                     val = os.path.abspath(val)
#                 args.append(val)
#             self._execute_script(script, args)
#             win.destroy()
#
#         tk.Button(win, text="Запустить", command=run_with_params).grid(columnspan=2, pady=5)
#
...
#         elif lang == "bash":
#             cmd = ["bash", script["path"], *args]
#         elif lang == "powershell":
#             cmd = ["powershell", "-File", script["path"], *args]
#         else:
#             messagebox.showerror("Ошибка", f"Неизвестный язык: {lang}")
#             return
#
...
class ParamDialog(tk.Toplevel):
    def __init__(self, parent, on_save=None, edit=None):
        ...
        ttk.OptionMenu(self, self.type_var, self.type_var.get(), "строка", "число", "список строк", "список чисел", "файловый путь").pack(fill=tk.X, padx=6)
